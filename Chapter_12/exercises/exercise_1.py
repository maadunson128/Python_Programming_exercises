...
class Dice:
    def __init__(self):
        '''Dice values initiation'''
        self.dice = [0] * 5
        self.rollAll()

    def roll(self, dice_list):
        '''Roll paricular dices'''
        for elm in dice_list:
            self.dice[elm] = randrange(1,7)

    def rollAll(self):
        '''Rolls all the dices at once'''
        self.roll(range(5))
...
def poker():
    ...
    win = GraphWin("PokerGame", 500, 500)
    text = Text(Point(250, 150), "This is video Poker game. \nClick click Lets'Play button to play Poker game")
    text.draw(win)

    b1 = Button(win, Point(150, 400), 100, 30, "Let's Play") #lets play button
    b1.activate()

    b2 = Button(win, Point(350, 400), 100, 30, "Quit") #Quit button
    b2.activate()
