...
def getDetails():
    ...
    sorting_type = input("Enter the type of sorting (ascending or descending): ")
    outfile = input("Enter the filename for sorted students namelist to stored: ")

    return infile, sort_param, sorting_type, outfile
...
def main():
    ...
    infile, sort_param, sort_type, outfile = getDetails()
    ...
    data.sort(key = sort_values[sort_param], reverse= sort_values[sort_type])
